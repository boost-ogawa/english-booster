...
if st.session_state.page == 0:
    ...
    if audio_url and audio_url.strip() != "":
        ...
        try:
            ...
            # エラーが起きても、アプリの進行は止めず、次の問題へ進むボタンは表示する
            st.markdown("---")
            if st.button("次の問題へ進む"):
                st.session_state.page = 5 # ページ5へ遷移
                st.rerun()
            # ここで st.stop() を入れると、以下の「原文」表示や「次の問題へ進む」ボタンが表示されないので注意
            # st.stop() # エラーでこれ以上進まないように停止 (今回のケースでは推奨しない)
        ...
        # audio_urlがNoneまたは空文字列の場合
        st.warning("この英文には音声データがありません。")
        # st.write でも良いですが、スタイル適用のため st.markdown を使うのが統一的です
        st.markdown(
            f"""
            <div class="custom-paragraph">
            {main_text}
