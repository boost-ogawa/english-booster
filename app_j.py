...
if st.session_state.is_authenticated:
    ...
    if user_profile_doc.exists:
        ...
        st.session_state.watched_videos = user_profile_data.get("watched_videos", [])
    else:
        st.session_state.enrollment_date = None
        st.session_state.watched_videos = []
            ...
            if st.button("登録日を設定", key="set_enrollment_date_button"):
                if target_nickname:
                    ...
                    target_user_profile_ref.set(
                        {"enrollment_date": enrollment_date_str, "watched_videos": firestore.ArrayUnion([])},
                        merge=True
                    )
    ...
    # 問題ページ (page 3)
        ...
        with col2:
            ...
            q1_options = data['Q1_options'].split(';')
            q2_options = data['Q2_options'].split(';')

            q1_choice = st.radio(data["Q1"], q1_options, key="q1")
            q2_choice = st.radio(data["Q2"], q2_options, key="q2")
    ...
    with col1:
        ...
        st.button("次へ", on_click=handle_login)